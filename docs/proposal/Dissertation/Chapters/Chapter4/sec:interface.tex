...
\begin{figure}
	\begin{center}
		\includegraphics{Chapter_4/interface_2D3D}
	\end{center}
...
\begin{algorithm}[H]
	...
	\label{alg:G_matrix}
\end{algorithm}
where \(s_i\) is one of the coupled structures, \(n^{\Gamma}\) and \(n^{s_i}\) are nodes number of the interface and nodes number of the structure \(s_i\), respectively; \(J_{\kappa}\) is the Jacobian evaluated at \((\xi_{\kappa},\eta_{\kappa})\) and \(N_{\kappa+1}\) is the shape function evaluated at \((\xi_{\kappa+1},\eta_{\kappa+1})\), \(n^{X_e}\) is the vector of global order numbers of all nodes in the \(ownerElements^j_i\), \(h_i\) is a thickness of the structure \(s_i\) and \(tol\) is a termination criterion for iterations.
...
\begin{figure}[H]
	\begin{center}
		\includegraphics{Chapter_4/b_b_test}
	\end{center}
