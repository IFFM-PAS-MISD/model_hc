%% APPENDIX HEADER ////////////////////////////////////////////////////////////////////////////////////
\chapter[Effective properties of the skin and the core]{Effective properties of the \acl{cfrp} skin and the homogenised aluminium core}
...
\begin{flalign}
	...
	&K_m=\frac{E_m}{2(1-\nu_m)},&\nonumber\\
	&G_m = \frac{E_m}{2(1+\nu_m)},&\nonumber\\
	&\nu_{23} = \nu_{12f}V_f+\nu_m(1-V_f)\frac{1+\nu_m-\nu_{12}\left(\frac{E_m}{E_{11}}\right)}{1-\nu^2_m+\nu_m\nu_{12}},&\nonumber\\
...
\end{flalign}
where \(m_f=\sin(\Theta_f)\) and \(n_f=\cos(\Theta_f)\).
...
\begin{flalign}
	&\rho = \rho_{al}\left[1-\frac{l_bn_h\left(h_b+l_bm_h\right)}{\left(l_2n_h+\mathrm{w}_c\right)\left(h_2+l_2m_h\right)}\right],&\\
	&E_{11} = k_1E_{al}\frac{(\mathrm{w}_c/l_b)^3n_h}{\left(h_2/l_2+m_h\right)m_h^2},\ E_{22} = k_2E_{al}\frac{(\mathrm{w}_c/l_b)^3(h_2/l_2+m_h)}{n_h^3},&\nonumber\\
	&E_{33} = E_{al}\left(\frac{\rho}{\rho_{al}}\right),&\nonumber\\
	&G_{12} = E_{al}\left(\frac{\mathrm{w}_c}{l_b}\right)^3\frac{(h_2/l_2+m_h)}{(h_b/l_b)^2n_hC},&\nonumber\\
	&G_{13} = G_{al}\left[\frac{\mathrm{w}_c/l_2}{(h_2/l_2+m_h)(\mathrm{w}_c/l_2)}\right]\left[n_h^2\frac{l_b}{l_2}+2\frac{\mathrm{w}_c}{l_2} \tan\left(\frac{\pi}{4}-\frac{\Theta_h}{2}\right)\right],&\nonumber\\
	&G_{23} = G_{al}\frac{\mathrm{w}_c/l_2}{\left(h_2/l_2+m_h\right)(\mathrm{w}_c/l_2 + n_h)} \left[m_h^2\frac{l_b}{l_2}+\frac{h_2}{l_2}+\frac{\mathrm{w}_c}{l_2}\tan\left(\frac{\pi}{4}-\frac{\Theta_h}{2}\right)\right],&\nonumber\\
	&\nu_{12} = n_h^2/(\frac{h_2}{l_2}+m_h)/m_hc_{12}k_1,\ \nu_{23} = \nu_{al}\frac{E_{22}}{E_{33}},&\nonumber
\end{flalign}
where
\begin{flalign}
	&\Theta_h = \arctan\left(\frac{h_1-h_2}{l_1}\right),\ m_h = \sin\Theta_h,\, n_h = \cos\Theta_h,&\nonumber\\
	...
	&k_2 = \frac{1}{1+(2.4+1.5\nu_{al}+\tan^2\Theta_h+h_b/(l_bn_h^2))(\mathrm{w}_c/l_b)^2},&\nonumber\\
	&c_{12} = 1+(1.4+1.5\nu_{al})(\mathrm{w}_c/l_b)^2,&\nonumber
\end{flalign}
\begin{flalign}
	\begin{split}
	C &= 1+2\left(\frac{h_b}{l_b}\right)+\left(\frac{\mathrm{w}_c}{l_b}\right)^2\times\\
	&\left[\frac{2.4+1.5\nu_{al}}{h_b/l_b}\left(4+\frac{h_2}{l_2}+m_h\right)+\frac{h_2/l_2+m_h}{\left(h_b/l_b\right)^2}\left(\left(h_2/l_2+m_h\right)\tan^2\Theta_h+m_h\right)\right].
	\end{split}
\end{flalign}
