...
function [prescribedPhi,Phi,q_e,groundNode,voltageNode] = piezo_function(structure_i)
...
q_e = cell(1,size(piezo_type,1));
[q_e{:}] = deal(zeros(numberNodes_piezo,1));
...
if ~isempty(piezo_type)
    ...
    prescribedPhi = sparse([],[],[],0,1,0);   Phi = sparse([],[],[],0,1,0);  q_e = sparse([],[],[],0,1,0);
