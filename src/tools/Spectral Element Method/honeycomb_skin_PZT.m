function [nodeCoordinates,elementNodes,cellPZT] = honeycomb_skin_PZT(structure,actSt)
...
for k=1:6
 p_0(k+1,:)=[R*cos(60*(k-1)*pi/180)-point_c(1,1),...
     R*sin(60*(k-1)*pi/180)-point_c(1,2)];
end
...
C1=0;
 ...
 for k=setdiff(1:size(structure,2),actSt)
     ...
     if ~isempty(structure(k).piezo_type)
         C4=C4+1;
%         if  ismember(k,structure(actSt).stAttach(1,:))
...
%% 
% jj=929;
%     for j=size(elementNodes{jj},1):-1:1
%         disp([j size(elementNodes{jj},1)])
%         disp(elementNodes{jj}(j,:))
% plot(nodeCoordinates([elementNodes{jj}(j,:) elementNodes{jj}(j,1)],1),...
%             nodeCoordinates([elementNodes{jj}(j,:) elementNodes{jj}(j,1)],2))        
%         plot(nodeCoordinates([elementNodes(j,:) elementNodes(j,1)],1),...
%             nodeCoordinates([elementNodes(j,:) elementNodes(j,1)],2))
%         text(nodeCoordinates(elementNodes(j,:),1)+0.0003,...
%             nodeCoordinates(elementNodes(j,:),2)-0.0003,...
%             num2str((elementNodes(j,:))'))
%         hold on
...
%         clc
%     end
%     for jj=1500:size(aa0,1)
%         plot(aa1(jj,1), aa1(jj,2),'b+')
%         hold on
%         plot(aa0(jj,1), aa0(jj,2),'ro')
%         axis([0.15 0.25 -0.25 -0.15])
%         axis equal
%         pause()
%     end
