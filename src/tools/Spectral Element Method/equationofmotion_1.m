function cmplCases = equationofmotion(N_f,Um,Ut,G,invd0,d1,M,MmC,invMpC,excit_sh,...
    ts,nr_exsh,q,Phi_electrode,parentFolder,name_project,case_name,Dof,GDof,nrNodesX,nrNodesZ,nrNodesY,stAttach2,...
    ...
    stiffness_V,inv_stiffness_V,Phi,activePhi,prescribedPhi,stiffness_uV,durationtime,output_result,intLay,...
    inputNumber,cmplCases)
...
for t = iSteps:N_f-1
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Fr = cell2mat(f) - cell2mat(KU) + 2/ts^2*M*cell2mat(Ut) - 1/ts^2*MmC*cell2mat(Um);
    if ~isempty(intLay)
      lambda = invd0*(d1*Fr); 
      Up = mat2cell(ts^2*invMpC*(Fr - cell2mat(G)'*lambda),cell2mat(GDof)',1);
    else
